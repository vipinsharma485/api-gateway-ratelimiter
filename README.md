...
> **Status:** Work in progress. See [Build Phases](#build-phases) for current progress.
...
| Downstream service | Spring Boot 3.2 (MVC) |
| Rate-limit state | Redis 7 (sliding window via Lua) |
| Event bus (planned) | Apache Kafka |
...
| 3 | Redis-backed fixed-window rate limiter | 🚧 In progress |
| 4 | Redis sliding-window via Lua script (atomic) | ⬜ |
...
| # | Decision | Status |
|---|----------|--------|
| [0001](docs/decisions/0001-distributed-rate-limiting.md) | Distributed rate limiting via Redis sliding window | Accepted |
